        ...
        <div className="space-y-6">
                ...
                <CardContent className="p-6">
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                        ...
                        <div className="flex-shrink-0">
                            {/* Case 1: Task is lockable based on role AND NOT yet locked by anyone */}
                            {((status === 'SUBMITTED' && (isOperator || isVerifier)) || (status === 'PENDING_VERIFICATION' && isVerifier)) &&
                                (!submission.current_assignee_id) && (
                            ...
                            {submission.current_assignee_id && submission.current_assignee_id !== user.id && (
                                <div className="flex flex-col items-end">
                                    <Badge variant="warning" icon={Lock} className="text-sm py-1.5 px-3">
                                        Sedang Dikunci
                                    </Badge>
                                    <span className="text-[10px] text-slate-500 mt-1 italic">
                                        Oleh: {submission.assignee?.full_name || 'Petugas Lain'}
                                    </span>
                                </div>
                            )}
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    {submission.current_assignee_id === user.id && (
                        ((status === 'SUBMITTED' || status === 'PROCESSING') && (isOperator || isVerifier)) ||
                        ...
                        /* Show "Kunci dulu" message if it's lockable for THIS role but NOT locked by me */
                        (((status === 'SUBMITTED' && (isOperator || isVerifier)) || (status === 'PENDING_VERIFICATION' && isVerifier)) && submission.current_assignee_id !== user.id) ? (
